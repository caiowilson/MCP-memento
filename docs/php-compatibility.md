...
The structural and Composer gates are intentionally exact. Framework relationship thresholds leave a narrow allowance for conservative omissions. Sixty-four natural-language retrieval queries with 69 answer-line judgments are scored independently within their corpus using the versioned `terms-v5` adapter. The evaluator reports 34 training, 11 validation, and 19 advisory holdout queries; validation and holdout judgments use range-bounded hard negatives. Training and validation are blocking at 95% recall@5, 90% MRR and nDCG@5, and zero hard-negative wins. The original 11-query holdout established a terms-v3 baseline of recall@5 `0.909`, MRR `0.773`, nDCG@5 `0.808`, and one hard-negative win. After terms-v4 was frozen at `cffc091`, that same generation scored recall@5 `1.000`, MRR `0.955`, nDCG@5 `0.966`, and zero hard-negative wins. An isolated agent then authored eight new cases without access to the scorer, suite, prior queries, history, or evaluator; terms-v4 ranked all eight first in isolation, but adding their files exposed a tie in an existing `never`-termination training query. That training miss was promoted into terms-v5 before a new terms-v5 holdout was authored. The PHP 7.4–8.4 roots include distractor files so language-version retrieval is not a one-file check.
