...
The structural and Composer gates are intentionally exact. Framework relationship thresholds leave a narrow allowance for conservative omissions. Eighty-five natural-language retrieval queries with 90 answer-line judgments are scored independently within their corpus using the versioned `terms-v8` adapter. The evaluator reports 39 training, 11 validation, and 35 advisory holdout queries; validation and holdout judgments use range-bounded hard negatives. Independently authored Composer packages use a retrieval-only holdout corpus so equally valid package mappings cannot invalidate base-package training labels. Training and validation are blocking at 95% recall@5, 90% MRR and nDCG@5, and zero hard-negative wins. The original 11-query holdout established a terms-v3 baseline of recall@5 `0.909`, MRR `0.773`, nDCG@5 `0.808`, and one hard-negative win. After terms-v4 was frozen at `cffc091`, that same generation scored recall@5 `1.000`, MRR `0.955`, nDCG@5 `0.966`, and zero hard-negative wins. An isolated post-terms-v4 generation ranked all eight first in isolation but exposed a tie in an existing `never`-termination training query when added to the full corpus, leading to terms-v5. A second isolated six-query generation then recorded recall@5 `1.000`, MRR `0.917`, nDCG@5 `0.938`, and one hard-negative win; its deferred-callable paraphrase miss was promoted before terms-v6. The first blind four-query terms-v6 generation recorded recall@5 `0.750`, MRR `0.625`, nDCG@5 `0.658`, and one hard-negative win. Its framework-neutral parent-to-collection miss is now a terms-v7 training case, activated only when paired association roles occur in the query and concrete ORM relationship syntax occurs in the chunk. The first blind five-query terms-v7 generation recorded recall@5 `0.800`, MRR `0.567`, nDCG@5 `0.626`, and two hard-negative wins: its Doctrine association and Composer mapping ranked first, while shutdown callback, backed-enum definition, and WordPress uninstall registration queries exposed new definition-versus-consumer gaps. Those three misses are now terms-v8 training cases with separate syntax-gated intents. The final blind six-query terms-v8 generation recorded recall@5 `1.000`, MRR `0.722`, nDCG@5 `0.794`, and three hard-negative wins. Full-suite training scores recall@5 `1.000`, MRR `1.000`, nDCG@5 `0.998`, and zero hard-negative wins; validation is perfect with zero hard-negative wins; advisory holdout scores recall@5 `1.000`, MRR `0.924`, nDCG@5 `0.944`, and three hard-negative wins. The PHP 7.4–8.4 roots include distractor files so language-version retrieval is not a one-file check.
...
1. Evaluate a bounded relationship-provider reranker that only promotes lexically matched candidates through explicit direct edges, with the production MCP path and evaluator sharing the same adapter.
2. Add an injected intent provider for definition-versus-consumer ordering, keeping deterministic terms-v8 as the offline fallback and evaluating semantic decisions with isolated advisory corpora.
