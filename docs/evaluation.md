...
Baseline refresh rationale for server v1.0.0: the terms-v12 indexing additions and Git-first change-detector tests moved four exact judgments in `indexer.go`, `server_test.go`, and the output-budget documentation. The ranges were re-audited against the same declarations, reference comment, metadata assertion, and client-budget passages. Only the fixture fingerprint changed; lexical metrics remain precision@5 `0.5`, recall@5 and MRR `1.0`, and macro nDCG `0.9765063588836707`, with no retrieval scoring or chunking change.
