...
By default (`MEMENTO_CHANGE_DETECTOR=auto`) the indexer uses resource-light Git polling for Git worktrees, including linked or isolated worktrees, and falls back to a recursive filesystem watcher only when Git is unavailable. Git polling remembers dirty paths so repeated edits and transitions back to clean content are reindexed. You can force a specific strategy with `MEMENTO_CHANGE_DETECTOR=fs` (filesystem watcher first) or `MEMENTO_CHANGE_DETECTOR=git` (Git polling first). See `docs/adr/ADRs.md`.
