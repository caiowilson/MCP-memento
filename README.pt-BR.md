...
- **Centralize revisões nas mudanças do diretório de trabalho.** Chame `repo_diff_context` sem `paths` para detectar automaticamente mudanças Git preparadas, não preparadas e não rastreadas, ou informe uma lista ordenada e não vazia de caminhos para substituir a detecção. A ferramenta retorna chunks apenas desses arquivos e um resumo unificado de diff limitado e redigido, informa excedentes e resume caminhos excluídos ou que são origem de renomeação sem carregar seus chunks nem expandir o grafo de relacionamentos.
