...
func TestTermsV12PostFreezeHoldout(t *testing.T) {
	suite, err := phpcompat.Load(filepath.Join("..", "..", "evaluation", "php-compat", "suite.v2.json"))
	if err != nil {
		t.Fatal(err)
	}
	const wanted = "postv12-php81-invoice-delivery-state-tokens"
	var selected phpcompat.Corpus
	for _, corpus := range suite.Corpora {
		for _, query := range corpus.Retrieval {
			if query.ID == wanted {
	...
	if len(selected.Retrieval) != 1 {
		t.Fatalf("post-terms-v12 holdout %q was not found", wanted)
	}
	root, err := suite.CorpusRoot(selected)
	if err != nil {
		t.Fatal(err)
	...
	if len(report.Queries) != 1 {
		t.Fatalf("post-terms-v12 results = %d, want 1", len(report.Queries))
	}
	result := report.Queries[0]
	wins := hardNegativeWins(retrievalQueriesByID(selected)[wanted], result.Retrieved)
	t.Logf("post-terms-v12 holdout recall@5=%.3f MRR=%.3f nDCG@5=%.3f hard-negative-wins=%d retrieved=%#v", result.Metrics.Recall, result.Metrics.MRR, result.Metrics.NDCG, wins, result.Retrieved)
}
