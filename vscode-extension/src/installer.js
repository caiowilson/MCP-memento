...
async function ensureServerInstalled(context) {
    ...
    await vscode.window.withProgress({
        ...
        if (!asset) {
            throw new Error(`No matching binary asset found in ${repo}@${release.tag_name}. Looked for: ${wanted.join(", ")}`);
        }
...
async function fetchRelease(repo, tag) {
    ...
    const url = tag === "latest" ? `${base}/releases/latest` : `${base}/releases/tags/${encodeURIComponent(tag)}`;
