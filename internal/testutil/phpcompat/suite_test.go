...
func TestCheckedInSuiteIsStrictAndComplete(t *testing.T) {
	...
	if suite.Version != 2 || suite.RetrievalPolicy.Adapter != "terms-v10+php-relationships-v1" || suite.RetrievalPolicy.K != 5 || !reflect.DeepEqual(suite.RetrievalPolicy.BlockingSplits, []string{RetrievalSplitTrain, RetrievalSplitValidate}) {
	...
	if retrievalQueries != 92 || retrievalJudgments != 98 || retrievalSplits[RetrievalSplitTrain] != 43 || retrievalSplits[RetrievalSplitValidate] != 11 || retrievalSplits[RetrievalSplitHoldout] != 38 {
