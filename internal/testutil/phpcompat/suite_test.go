...
func TestCheckedInSuiteIsStrictAndComplete(t *testing.T) {
	...
	if suite.Version != 2 || suite.RetrievalPolicy.Adapter != "terms-v11+php-relationships-v1" || suite.RetrievalPolicy.K != 5 || !reflect.DeepEqual(suite.RetrievalPolicy.BlockingSplits, []string{RetrievalSplitTrain, RetrievalSplitValidate}) {
	...
	if retrievalQueries != 93 || retrievalJudgments != 99 || retrievalSplits[RetrievalSplitTrain] != 44 || retrievalSplits[RetrievalSplitValidate] != 11 || retrievalSplits[RetrievalSplitHoldout] != 38 {
