...
func TestCheckedInSuiteIsStrictAndComplete(t *testing.T) {
	...
	if suite.Version != 2 || suite.RetrievalPolicy.Adapter != "terms-v15+php-relationships-v1" || suite.RetrievalPolicy.K != 5 || !reflect.DeepEqual(suite.RetrievalPolicy.BlockingSplits, []string{RetrievalSplitTrain, RetrievalSplitValidate}) {
	...
	wantIDs := []string{
		...
		"postv15-php81-orchard-frost-response",
		"symfony-app",
		"wordpress-holdouts",
	...
	if retrievalQueries != 97 || retrievalJudgments != 103 || retrievalSplits[RetrievalSplitTrain] != 48 || retrievalSplits[RetrievalSplitValidate] != 11 || retrievalSplits[RetrievalSplitHoldout] != 38 {
